...
Cross-platform Flutter + FastAPI application that produces printable 300 DPI A4 coloring pages for children. The backend builds a safe prompt, calls the OpenAI Images API (with a local fallback for development), converts it to bold line art, and exports a PDF. The Flutter client provides a simple, parent-friendly interface to pick a theme and age group and download the PDF.
...
backend/            # FastAPI service
  app/
    main.py         # API surface and static file hosting
...
Environment variables (place in a `.env` or export in your shell):

```
OPENAI_API_KEY=<your_api_key>  # optional for placeholder fallback
...
- Returns: `{ "status": "ok", "pdf_url": "/files/<name>.pdf", "preview_base64": "..." }` (A4 @ 300 DPI). Files are saved to `backend/output/` and served at `/files/...`.
